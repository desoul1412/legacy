...
if __name__ == "__main__":
    spark = SparkSession.builder.appName('L2M money spend').getOrCreate()
    ...
    existing_dates_df = (
        ...
        .option("dbtable", "(SELECT DISTINCT report_date FROM public.money_spend) t")
        .option("user", DBUSER)
    ...
    for d in pending_dates:
        ...
        df = (
            spark.read
                ...
                .option("dbtable", money_spend_query)
                .option("user", "sonph4")
        ...
        daily_spend = (
            ...
            .withColumn("spend_amount", F.coalesce(F.col("use_num2"), F.lit(0)))
            .join(user_profile, how="left", on="user_id")
            .select("report_date","user_id","server","reason_id","spend_amount","country_code","vip_level")
        ...
        daily_spend.write.format("jdbc") \
            .mode("append") \
            .option("url", f"jdbc:postgresql://{DBHOST}:{DBPORT}/{DBNAME}") \
            .option("dbtable", "public.money_spend") \
            .option("user", DBUSER) \
            .option("password", DBPASS) \
            .option("driver", "org.postgresql.Driver") \
            .save()
