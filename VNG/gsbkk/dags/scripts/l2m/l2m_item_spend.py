...
if __name__ == "__main__":
    spark = SparkSession.builder.appName('L2M Item spend').getOrCreate()
    ...
    existing_dates_df = (
        ...
        .option("dbtable", "(SELECT DISTINCT report_date FROM public.spend) t")
        .option("user", DBUSER)
    ...
    for d in pending_dates:
        item_spend_query = f"""(
            SELECT
                ...
                use_num1
            FROM iceberg.l2m.etl_decrease_item
        ...
        df = (
            spark.read
                ...
                .option("dbtable", item_spend_query)
                .option("user", "sonph4")
        ...
        daily_spend = (
            ...
            .withColumn("spend_amount", F.coalesce(F.col("use_num1"), F.lit(0)))
            .join(user_profile, how="left", on="user_id")
            .select("report_date","user_id","server","role_id","role_level","item_id","spend_amount","country_code")
            .groupby("report_date","server","role_level","country_code","item_id")
            .agg(F.sum("spend_amount").alias("spend_amount"))
        )

        daily_spend.write.format("jdbc") \
            .mode("append") \
            .option("url", f"jdbc:postgresql://{DBHOST}:{DBPORT}/{DBNAME}") \
            .option("dbtable", "public.spend") \
            .option("user", DBUSER) \
            .option("password", DBPASS) \
            .option("driver", "org.postgresql.Driver") \
            .save()
